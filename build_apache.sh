...
then
	...
	./config --prefix=/opt/openssl --openssldir=/opt/openssl no-blowfish no-ssl3 no-ec2m no-rc5 no-idea no-camellia no-des no-weak-ssl-ciphers enable-ec_nistp_64_gcc_128 threads zlib-dynamic shared
	make depend
	make
	sudo make install
fi
...
./configure --prefix=/opt/apache2 --enable-pie --enable-mods-shared=all --enable-so --disable-include --enable-lua --enable-deflate \
	...
	--enable-mpms-shared=all --with-mpm=event --enable-rewrite --with-z=${HOME}/apache24/httpd-${HTTPD_VERSION}/srclib/zlib --enable-fcgid --with-included-apr
make
sudo make install
